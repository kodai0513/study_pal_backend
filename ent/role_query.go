...
func (rq *RoleQuery) QueryWorkbookMembers() *WorkbookMemberQuery {
	query := (&WorkbookMemberClient{config: rq.config}).Query()
...
func (rq *RoleQuery) WithWorkbookMembers(opts ...func(*WorkbookMemberQuery)) *RoleQuery {
	query := (&WorkbookMemberClient{config: rq.config}).Query()
...
func (rq *RoleQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Role, error) {
	...
	if query := rq.withWorkbookMembers; query != nil {
		if err := rq.loadWorkbookMembers(ctx, query, nodes,
			func(n *Role) { n.Edges.WorkbookMembers = []*WorkbookMember{} },
			func(n *Role, e *WorkbookMember) { n.Edges.WorkbookMembers = append(n.Edges.WorkbookMembers, e) }); err != nil {
			return nil, err
		}
	}
...
func (rq *RoleQuery) loadWorkbookMembers(ctx context.Context, query *WorkbookMemberQuery, nodes []*Role, init func(*Role), assign func(*Role, *WorkbookMember)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(workbookmember.FieldRoleID)
	}
